...
assert "configsPerJob" in parsed and parsed["configsPerJob"] > 0, "configsPerJob not provided"
...
if parsed["configsPerJob"] == 1:
    sbatchContent += f"#SBATCH --array=0-{len(combinations) - 1}"
else:
    # TODO
    raise NotImplementedError()
sbatchContent += f"%{parsed['maxSimultaneousJobs']}\n"
...
sbatchContent += "scontrol update JobId=$SLURM_JOB_ID JobName=$ezgrid_id\n\n"
...
console.print(f"Number of configurations per job: [bold yellow]{parsed['configsPerJob']}[/]")
console.print(f"Maximum number of simultaneous jobs: [bold yellow]{parsed['maxSimultaneousJobs']}[/]")
console.print("")
console.input("Press enter to continue...")
console.print("")
